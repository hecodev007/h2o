...
contract Treasury is ContractGuard, Epoch, Orchestrator {
    ...
    address public fund;
    address public cash;
    address public sCarrot;
    address public boardroomTimeLock;

    address public seigniorageOracle;
    // uint256 public timePeriod = 21600; // 6 hour
    uint256 public timePeriod = 180; // 3 min
    
    // ========== PARAMS
    uint256 public cashPriceOne =  10**18;
    uint256 private accumulatedSeigniorage = 0;
    uint256 public fundAllocationRate = 10;
    uint256 public IssuanceRatio = 70;
    ...
    constructor(
        ...
        address _sCarrot,
        uint256 _startTime
    ) public Epoch(15 minutes, _startTime, 0) {
        cash = _cash;
    ...
    function migrate(address target) public onlyOperator checkOperator {
        ...
        // sCarrot
        Operator(sCarrot).transferOperator(target);
        Operator(sCarrot).transferOwnership(target);
        IERC20(sCarrot).transfer(target, IERC20(sCarrot).balanceOf(address(this)));
    ...
    /* ========== MUTABLE FUNCTIONS ========== */
    function buysCarrots(uint256 amount)
        external
        onlyOneBlock
        checkMigration
        checkStartTime
        checkOperator
    {
        require(amount > 0, 'Treasury: cannot purchase sCarrot with zero amount');
        IERC20(cash).transferFrom(msg.sender,address(this),amount);
        IBasisAsset(sCarrot).mint(msg.sender, amount);
        emit BoughtsCarrots(msg.sender, amount);
    }

    function redeemsCarrots(uint256 amount)
        external
        onlyOneBlock
        checkMigration
        checkStartTime
        checkOperator
    {
        require(amount > 0, 'Treasury: cannot redeem sCarrots with zero amount');
        require(_getCashPrice(seigniorageOracle) >= cashPriceOne,"Treasury: Need price greater than 1");
        require(
        ...
        IBasisAsset(sCarrot).burnFrom(msg.sender, amount);
        IERC20(cash).safeTransfer(msg.sender, amount);
        emit RedeemedsCarrots(msg.sender, amount);
    }
        ...
        checkOperator
    {
        ...
        if (IERC20(sCarrot).totalSupply() > accumulatedSeigniorage){
            treasuryReserve = Math.min(
                seigniorage,
                IERC20(sCarrot).totalSupply().sub(accumulatedSeigniorage));
        }
        ...
        if (boardroomReserve > 0) {
            // boardroomTimeLock
            IERC20(cash).safeApprove(boardroomTimeLock, boardroomReserve);
            IBoardroom(boardroomTimeLock).allocateSeigniorage(boardroomReserve);
    ...
    // CORE
    event RedeemedsCarrots(address indexed from, uint256 amount);
    event BoughtsCarrots(address indexed from, uint256 amount);
